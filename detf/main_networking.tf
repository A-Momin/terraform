...
resource "aws_route_table" "detf_vpc_rt_public" {
  ...
  route {
    cidr_block = "0.0.0.0/0"                           # destination arguments
    gateway_id = aws_internet_gateway.detf_vpc_igw1.id # target arguments
  }
}
# resource "aws_route" "public_internet_access" {
#   route_table_id         = aws_route_table.detf_vpc_rt_public.id
#   destination_cidr_block = "0.0.0.0/0"                           # destination arguments
#   gateway_id             = aws_internet_gateway.detf_vpc_igw1.id # target arguments
# }
